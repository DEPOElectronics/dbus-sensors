...
void i2cSensor::setupRead(void)
{
    ...
    std::weak_ptr<i2cSensor> weak = weak_from_this();
    ...
    inputDev.async_read_some_at(
        ...
        [weak, buffer{buffer}](const boost::system::error_code& ec,
                               size_t bytesRead) {
        std::shared_ptr<i2cSensor> self = weak.lock();
        if (!self)
        {
            return;
        }
...
void i2cSensor::restartRead(void)
{
...
void i2cSensor::handleResponse(const boost::system::error_code& err,
                               size_t bytesRead)
{
